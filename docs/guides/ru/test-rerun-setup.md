...
Veriqorn позволяет перезапускать упавшие или выбранные тесты прямо со страницы запуска. Новые результаты заменяют старые **в том же запуске** — отдельные дочерние запуски не создаются.
...
> **Безопасность исходящих подключений:** Добавьте hostname каждого CI-webhook и test-rerun agent в `OUTBOUND_ALLOWED_HOSTS` окружения backend. Запросы к хостам вне allowlist блокируются; указывайте только hostname, без схемы и пути.
