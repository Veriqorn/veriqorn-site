...
> **Безопасность исходящих подключений:** перед настройкой коннектора добавьте его hostname в `OUTBOUND_ALLOWED_HOSTS` окружения backend. Например: `OUTBOUND_ALLOWED_HOSTS=sentry.io,grafana.internal.example,kibana.internal.example`. Неразрешённые хосты блокируются, а тест подключения не возвращает тело ответа сервиса.
