...
> **Безопасность исходящих подключений:** до настройки LLM добавьте hostname провайдера в `OUTBOUND_ALLOWED_HOSTS` окружения backend, например `OUTBOUND_ALLOWED_HOSTS=api.openai.com,ollama.internal.example`. Запросы к неразрешённым хостам блокируются. Указывайте только hostname без схемы и пути; внутренние сервисы нужно перечислить явно. После изменения перезапустите backend.
