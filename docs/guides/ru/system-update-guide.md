...
Для обычных обновлений используйте **Настройки → Обновления платформы** после
однократной настройки агента ниже. Ручной сценарий в этом гайде нужен для
контролируемого обслуживания, фиксации конкретного Community-релиза или
восстановления.
...
Для канонического набора переменных окружения ориентируйтесь на `veriqorn/deploy/.env.example`.
...
## Шаг 2 - Выберите версию для ручного обновления (дополнительно)
...
## Шаг 3 - Загрузите и примените ручное обновление
...
## Рекомендуемый путь: обновление из интерфейса
