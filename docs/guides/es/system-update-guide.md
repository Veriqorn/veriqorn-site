...
Utilice este flujo cuando ya tenga la plataforma en ejecucion con `docker-compose.install.yml`.
...
- Un despliegue en ejecucion basado en `docker-compose.install.yml`
- Acceso a `.env.install`
...
Si utiliza credenciales o nombre de base de datos personalizados, reemplace `postgres` y `test_ops` con los valores de su `.env.install`.
...
Establezca la version de la plataforma en `.env.install`:
...
1. Establezca la version anterior en `.env.install` (por ejemplo `PLATFORM_VERSION=v1.1.0`).
2. Ejecute:
...
- la clave de verificacion se lee desde `AI_ANALYSIS_LICENSE_PUBLIC_KEY` en `.env.install`
...
- Mantenga `.env.install` en respaldo y gestion de secretos.
