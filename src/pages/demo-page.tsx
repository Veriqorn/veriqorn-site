...
export function DemoPage() {
  ...
  const [view, setView] = useState<"overview" | "launches" | "launch" | "results" | "test" | "coverage" | "knowledge" | "settings">("launches");
          ...
          <main className="mx-auto w-full max-w-[1600px] p-4 md:p-6">
            ...
            <div className="grid gap-6 2xl:grid-cols-[minmax(0,1.45fr)_minmax(340px,0.85fr)]">
              <section className="overflow-hidden rounded-xl border border-[#d9d0c4] bg-white/95 shadow-[0_18px_50px_-28px_rgba(15,23,42,0.28)]">
                <div className="flex flex-col gap-3 border-b border-[#e8e0d7] p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-semibold text-[#172337]">Recent launches</h2><p className="mt-0.5 text-xs text-slate-500">Select a launch to inspect its test results.</p></div><div className="flex items-center gap-2"><Filter className="h-4 w-4 text-slate-400" />{(["all", "failed", "broken", "passed"] as const).map((filter) => <button key={filter} onClick={() => setStatusFilter(filter)} className={cn("rounded-lg px-2.5 py-1.5 text-xs font-semibold capitalize", statusFilter === filter ? "bg-[#16788a] text-white shadow-sm" : "border border-transparent text-slate-500 hover:border-[#d9d0c4] hover:bg-[#f4ede4]")}>{filter === "all" ? "All" : statusText[filter]}</button>)}</div></div>
...
function DemoSubPage({ view, run, test, onNavigate, onTestSelect }: {
  ...
  if (view === "overview") return <div className="grid gap-6 xl:grid-cols-3"><div className={cn(panel, "xl:col-span-2")}><p className="eyebrow">Quality overview</p><h2 className="mt-2 text-xl font-semibold text-[#172337]">Your project is stable</h2><div className="mt-6 grid grid-cols-3 gap-4"><Metric label="Pass rate" value="93%" detail="Last 30 days" icon={CheckCircle2} tone="emerald" /><Metric label="Flaky rate" value="2.8%" detail="-0.6% this week" icon={AlertTriangle} tone="rose" /><Metric label="Launches" value="48" detail="This month" icon={Rocket} tone="cyan" /></div><button onClick={() => onNavigate("launches")} className="mt-6 rounded-xl bg-[#16788a] px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-[#106676]">View launches</button></div><div className={panel}><p className="eyebrow">AI insight</p><div className="mt-4 flex gap-3"><Bot className="h-5 w-5 text-[#16788a]" /><p className="text-sm leading-6 text-slate-600">Payment tests began failing after the latest provider-client update. Open a launch to inspect the evidence.</p></div></div></div>;
  ...
  if (view === "results") return <section className={panel}><div className="flex items-center justify-between"><div><p className="eyebrow">{run.title}</p><h2 className="mt-1 text-xl font-semibold text-[#172337]">Test results</h2></div><div className="rounded-xl border border-[#d9d0c4] px-3 py-2 text-xs text-slate-500"><Search className="mr-2 inline h-3.5 w-3.5" />Search results</div></div><div className="mt-6 divide-y divide-[#e8e0d7]">{tests.map((item) => <button key={item.name} onClick={() => onTestSelect(item)} className="flex w-full items-center gap-3 py-4 text-left hover:bg-[#faf8f5]"><StatusIcon status={item.status} className={cn("h-5 w-5", item.status === "failed" ? "text-rose-500" : item.status === "broken" ? "text-amber-500" : "text-slate-400")} /><span className="flex-1"><b className="block text-sm text-[#172337]">{item.name}</b><span className="text-xs text-slate-500">{item.suite}</span></span><span className="text-xs text-slate-400">{item.duration}</span><ChevronRight className="h-4 w-4 text-slate-400" /></button>)}</div></section>;
  ...
  const copy = view === "coverage" ? ["Coverage", "82.4%", "Coverage is calculated across 412 source files. Explore uncovered lines and recent trends."] : view === "knowledge" ? ["Knowledge Base", "Ask your project", "Search indexed test documentation, repositories, and team knowledge."] : ["Settings", "Project settings", "Configure integrations, test result imports, and access for this project."];
