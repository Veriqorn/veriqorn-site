...
export function DemoPage() {
  ...
  const navItems = [["Overview", LayoutDashboard], ["Launches", Play], ["Coverage", ShieldCheck], ["Knowledge Base", Bot], ["Settings", Settings2]] as const;
    ...
    <div className="demo-platform -mx-4 my-0 overflow-hidden border-x border-[#d9d0c4] sm:-mx-6 lg:-mx-8">
      <div className="flex min-h-[calc(100vh-9rem)] bg-[#faf8f5]">
        <aside className="hidden w-[20.5rem] shrink-0 flex-col border-r border-white/10 bg-[#172337] p-3 text-[#f2eee7] shadow-[28px_0_90px_rgba(9,16,30,0.28)] lg:flex">
          ...
          <nav className="flex-1 space-y-1">{navItems.map(([name, Icon]) => <button key={name} onClick={() => setActiveSection(name)} className={cn("flex w-full items-center gap-3 rounded-[1rem] border px-3 py-3 text-left text-sm font-medium transition-all", activeSection === name ? "border-white/12 bg-white/15 text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]" : "border-transparent text-white/60 hover:border-white/10 hover:bg-white/10 hover:text-white")}><Icon className="h-4 w-4" />{name}</button>)}</nav>
        ...
        <div className="min-w-0 flex-1">
          <main className="mx-auto w-full max-w-[1600px] p-4 md:p-6">
            <header className="relative overflow-hidden rounded-xl border border-[#d9d0c4] bg-white/95 px-6 py-6 shadow-[0_18px_50px_-28px_rgba(15,23,42,0.28)] sm:px-8"><div className="absolute inset-x-0 top-0 h-24 bg-[linear-gradient(90deg,rgba(7,166,184,0.12),rgba(248,128,88,0.12),transparent)]" /><div className="relative flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-3xl font-semibold tracking-tight text-[#172337]">{activeSection === "Launches" ? "Launches" : activeSection}</h1></div><div className="flex items-center gap-2"><span className="rounded-full border border-[#d9d0c4] bg-white px-3 py-1 text-xs font-medium text-slate-600">Acme Store</span><span className="rounded-full bg-emerald-600 px-3 py-1 text-xs font-semibold text-white">AI Pro</span><Link to="/" className="rounded-xl border border-[#d9d0c4] bg-white px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-[#f4ede4]">Exit demo</Link></div></div></header>
